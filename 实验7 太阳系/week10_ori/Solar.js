...
var hourOfDayX = 0.0;		// planet一天中的小时数，初始为0
var dayOfYearX = 0.0;		// 一年中的天数，初始为0
...
window.onload = function main(){
	...
	matProj = perspective(30.0, 		// 垂直方向视角
...
function drawXSystem(mvp){
	...
	mvp = mult(mvp, scale(0.5, 0.5, 0.5));// 控制地球大小的缩放变换
	// 画一个蓝色的球来表示地球
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
	gl.uniform3f(u_Color, 0.2, 0.8, 1.0 );  // 蓝色
...
//同步卫星
	...
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
	gl.uniform3f(u_Color, 0.8, 0.2, 1.0 ); 
	gl.drawArrays(gl.LINES, 0, numVertices);
	mvp = mvpStack.pop();
...
//逆向卫星
	mvp = mult(mvp, rotateY(360.0 *8* dayOfYearX / 300.0));
	mvp = mult(mvp, translate( 0.95, 0.0, 0.0 ));
	mvp = mult(mvp, rotateX(90));
	mvp = mult(mvp, scale(0.1, 0.1, 0.1));// 控制月球大小的缩放变换
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
	gl.uniform3f(u_Color, 0.8, 0.2, 0.2 ); 
	gl.drawArrays(gl.LINES, 0, numVertices);
}
...
function drawSolarSystem(mvp){
	...
	mvpStack.push(mvp);
	mvp = mult(mvp, rotateY(3600.0 * dayOfYear / 365.0)); // 将MVP矩阵压进栈
	mvpStack.push(mvp);
	//left
	mvp = mult(mvp, translate(-0.7, 0, 0));
	mvp = mult(mvp, rotateX(90));
	mvp = mult(mvp, scale(0.5, 0.5, 0.5)); // 控制太阳大小的缩放变换
	...
	mvpStack.push(mvp);
	mvp = mult(mvp, rotateY(360.0 * dayOfYearX / 300.0)); // 将MVP矩阵压进栈
	mvp = mult(mvp, translate(2.5, 0, 0));
	...
	// 用于控制地球和太阳之间距离的平移变换
    mvp = mult(mvp, translate(4.0, 0.0, 0.0));
	//40度
	mvp = mult(mvp, rotateY(-360.0 * dayOfYear / 365.0));
...
function drawMoonSystem(mvp){
	...
	mvp = mult(mvp, scale(0.1, 0.1, 0.1));// 控制月球大小的缩放变换
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
	gl.uniform3f(u_Color, 0.3, 0.7, 0.3 ); 
	gl.drawArrays(gl.LINES, 0, numVertices);
...
//绘制月球卫星
	...
	mvp = mult(mvp, rotateX(90));
	mvp = mult(mvp, scale(0.05, 0.05, 0.05));// 控制月球卫星大小的缩放变换
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
...
function drawEarthSystem(mvp){
	...
	// 画一个蓝色的球来表示地球
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp)); // 传模视投影矩阵
	gl.uniform3f(u_Color, 0.2, 0.2, 1.0 );  // 蓝色
	gl.drawArrays(gl.LINES, 0, numVertices);
	mvp = mvpStack.pop(); // 恢复矩阵状态
